...
export class TransactionFeesAddEditComponent implements OnInit, OnDestroy {
  ...
  private loadFeeData(id: number): void {
    const url = this.constant.API_ENDPOINT + `Fees/GetById/${id}`;
  ...
  submit(): void {
    ...
    if (this.isEditMode) {
      const url = this.constant.API_ENDPOINT + `Fees/Update/${this.feeId}`;
      ...
      const url = this.constant.API_ENDPOINT + 'Fees/Create';
