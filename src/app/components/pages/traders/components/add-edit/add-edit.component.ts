...
export class TradersAddEditComponent implements OnInit, OnDestroy {
  ...
  private loadTraderData(id: number): void {
    const url = this.constant.API_ENDPOINT + `Traders/GetById/${id}`;
  ...
  submit(): void {
    ...
    if (this.isEditMode) {
      const url = this.constant.API_ENDPOINT + `Traders/Update/${this.traderId}`;
      ...
      const url = this.constant.API_ENDPOINT + 'Traders/Create';
