...
export class PhonesAddEditComponent implements OnInit, OnDestroy {
  ...
  private initializeForm(): void {
    this.form = this.fb.group({
      ...
      Number: ['', [Validators.required, Validators.pattern(/^0[0-9]{10}$/)]]
    });
  }

  private loadPhoneData(id: number): void {
    const url = this.constant.API_ENDPOINT + `PhoneNumbers/GetById/${id}`;
  ...
  submit(): void {
    ...
    if (this.isEditMode) {
      const url = this.constant.API_ENDPOINT + `PhoneNumbers/Update/${this.phoneId}`;
      ...
      const url = this.constant.API_ENDPOINT + 'PhoneNumbers/Create';
