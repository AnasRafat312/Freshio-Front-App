...
export class BankAccountsAddEditComponent implements OnInit, OnDestroy {
  ...
  private loadBankAccountData(id: number): void {
    const url = this.constant.API_ENDPOINT + `BankAccounts/GetById/${id}`;
  ...
  submit(): void {
    ...
    if (this.isEditMode) {
      const url = this.constant.API_ENDPOINT + `BankAccounts/Update/${this.bankAccountId}`;
      ...
      const url = this.constant.API_ENDPOINT + 'BankAccounts/Create';
